...
<script>
    $(function(){
     ...
     ayudas('#','botonera-reparo','top right','bottom left','fold','up');
    ...
    $('#calcular_re').click(function() {
        $("#listar_reparo input[type=checkbox]").each(function() { 
...
//                    alert($(this).val());
                    $.ajax({
                        type:'post',
                        data:{valores:$(this).val(),opc_tipo_multa:2},
                        dataType:'json',
...
//                                alert('no'+div);
                             ...
                             $('#error-rep-clac').html('<p style="font-family: sans-serif;color:#CD0A0A;"><span style="float: left; margin-right: .3em;" class="ui-icon ui-icon-check"></span><strong>Aviso: </strong>Calculo realizado con exito.<br /><br /><center><i>Dirigase a calculos por aprobar si desea ver los detalles.</i></center></p>')
                             $("#error-rep-clac").addClass('ui-state-highlight ui-corner-all'); 
                             $("#error-rep-clac").css({background:'#FAF9EE',border:'1px solid #FEEE12'});
                             $("#error-rep-clac").show('slide',{ direction: "up" },1000);
                        }


                    });
                }else{
                    $('#error-rep-clac').html('<p style="font-family: sans-serif;color:#CD0A0A;"><span style="float: left; margin-right: .3em;" class="ui-icon ui-icon-alert"></span><strong>ERROR: </strong>Marque al menos una para calcular.</p>')
                    $("#error-rep-clac").addClass('ui-state-error ui-corner-all'); 
                    $("#error-rep-clac").css({background:'#FEF6F3',border:'1px solid #CD0A0A'});
                    $("#error-rep-clac").show('slide',{ direction: "up" },1000);
                }
...
//                    
         ...
         <?php if (!empty($data)){?> 
        <table id="botonera-reparo" border="0" style=" width: 100%" >
          <tr>
              <td style=" width: 750px">
                  &nbsp; &nbsp;&nbsp;
              </td>
              <td >
               <button txtayuda="Selecionar para calcular la multa" class="ayuda" id="selec_todos_re" style="width: 80px; height: 25px" value="marca" >Marcar</button>

              </td>
              <td >
               <button txtayuda="ejecutar la operacion de calculo" class="ayuda" id="calcular_re" style="width: 80px; height: 25px" value="calculo" >Calcular</button>
 
              </td>
          </tr>
      </table>
         <div id="error-rep-clac" style=" width: 300px; margin-left: 200px"></div>
         <?php };?>
<!--         <div id="botonera-reparo" style=" margin-left: 83%; margin-top:10px">
               <button txtayuda="Selecionar para calcular la multa" class="ayuda" id="selec_todos_re" style="width: 80px; height: 25px" value="marca" >Marcar</button>
               <button txtayuda="ejecutar la operacion de calculo" class="ayuda" id="calcular_re" style="width: 80px; height: 25px" value="calculo" >Calcular</button>
        </div>-->
...
//            script para asignar atributos al listar diseñado con datatables
            oTable = $('#listar_reparo').dataTable({
					"bJQueryUI": true,
