        ...
        carga_vista_inicio_backend()
         ...
         $( "#dialogarmamenu" ).dialog({    
             ...
             $( "#alerta-abuelo" ).dialog({    
               ...
               buttons: {
                    "SI": function() {
                        location.href="<?php print($base_url); ?>salida";
                        $( this ).dialog( "close" );          
...
carga_vista_inicio_backend=function(){
        $.ajax({
                dataType:"html",
                url:"<?php  echo base_url().'index.php/mod_administrador/principal_c/cargar_vista_inicio_backend/'; ?>",
...
<!--        <div id="menu-baner-home" style="width: 6%; height: 2%; position:absolute; margin-top: -32px; margin-left: 85%; border: 2px solid;">
            <div style="position: relative; border: 2px solid; width: ">
                Inicio
            </div>
            <div style="position: absolute; border: 2px solid;  float:left">
                Salir
            </div></div>-->


       
<!--     <span style=" color:#888888;margin-top:4%; float: left; margin-left:21%;"><b><?php // echo "Bienvenido(a), ".$primer_nombre;?></b></span>
  -->
...
<!--                <input type="checkbox" id="btn_main_toggle_menu" /><label for="btn_main_toggle_menu">Men&uacute;</label>-->
<!--                <button id="btn_main_exit" accesskey="x">Salir</button>-->
    ...
    <div id="alerta-abuelo" title="Mensaje">
        
        <p id="mensaje-abuelo" style="text-align:justify;">Para visualizar el modulo creado debe cerrar sesion</p>
        <p style="text-align:center;"><b>¿ desea cerrar la sesion?</b></p>
...
<!--<div style="padding: 0 .7em; width: 150px;"  id="cargando"  >
...
foreach( $info["info_modulos"] as $item):
    if( $item["id_padre"]==0 ):
        echo "$(\"#main_menu\").append( menu_crear_titulo(\"title-".$item["id_modulo"]."\",\"".utf8_decode(utf8_encode($item["str_modulo"]))."\",\"".$base_url."\",\"".$item["str_rol"]."\") );";
...
"<?php print(utf8_decode($base_url.$item["str_enlace"]."?padre=".$item["id_modulo"]."&nombrerol=".$item['str_rol']));?>",
...
//$("#btn_main_exit").button().click(function(){
...
monitorea_session=function(){    
    $.ajax({
           type:"post",
           dataType:"json",
           url:"<?php echo base_url().'index.php/inicio/monitorea_session'?>",
           success:function(data){
